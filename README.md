[Blox Fruits 自動打怪 Bot (C# WinForms + Python).md](https://github.com/user-attachments/files/30627790/Blox.Fruits.Bot.C.WinForms.%2B.Python.md)
...
透過 Named Pipes 與 Python 端進行通訊# Blox Fruits 自動打怪 Bot (C# WinForms + Python)
...
6.  **狀態機管理**：Bot 根據 Python 決策在 `GET_QUEST` (接任務)、`TRANSFORM` (變身)、`COMBAT` (戰鬥)、`NAVIGATE` (巡邏) 之間切換。
...
**注意事項**：
-   `python_client.py` 是一個模擬腳本，它會隨機生成決策並發送給 C# 端。您需要根據實際需求，在 `python_client.py` 中實作圖像識別和 AI 決策邏輯。
...
-   **Python AI 決策**：將 `python_client.py` 中的模擬決策替換為實際的圖像識別和 AI 邏輯，例如使用 OpenCV 識別遊戲元素、血條、能量條、目標等。
...
根據分析結果，生成 JSON 格式的決策（DecisionIntent），並回傳給 C# 端。
功能需求
自動尋找 Roblox 視窗句柄：透過視窗標題尋找 Roblox 遊戲視窗。
截取 Roblox 視窗畫面：定期截取遊戲畫面並保存為 PNG 圖片。
Named Pipe 通訊：C# 作為伺服器，Python 作為客戶端，實現雙向通訊。
JSON 決策處理：C# 端解析 Python 回傳的 JSON 決策，Python 端生成 JSON 決策。
後台按鍵發送：根據決策，使用 PostMessage 向 Roblox 視窗發送按鍵和滑鼠事件。
狀態機管理：Bot 根據 Python 決策在 GET_QUEST (接任務)、TRANSFORM (變身)、COMBAT (戰鬥)、NAVIGATE (巡邏) 之間切換。
日誌顯示：所有操作和狀態變化實時顯示在 WinForms 介面的日誌視窗中。
一鍵啟動/緊急停止：提供簡單的控制按鈕。
Python 端回傳的 JSON 格式
{
    "HpRatio": 0.85,
...
使用 Visual Studio 開啟 BloxFruitsBot.sln 解決方案。
建置 (Build) 專案以生成可執行文件。
執行 BloxFruitsBot.exe。
在 WinForms 介面中，點擊「開始」按鈕。
注意事項：
